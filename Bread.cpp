#include "Bread.h"

Bread::Bread(int units = 0, float price = 0, bool sale = false, bool gift = false)
{
	Units = units;
	setPrice(price);
	Sale = sale;
	Gift = gift;
	setName("Bread");
	setType("Food");
}

Bread::Bread(Bread& copy)
{
	Units = copy.Units;
	Sale = copy.Sale;
	Gift = copy.Gift;
	setPrice(copy.getPrice());
	setName(copy.getName());
	setType(copy.getType());
}

float Bread::price()
{
	if (Sale)
	{
		float newPrice = getPrice() * 25/100;
		newPrice = getPrice()- newPrice;
		return Units * newPrice;
	}
	return Units * getPrice();
}

bool Bread::sale()
{
	return Sale;
}

bool Bread::gift()
{
	return Gift;
}

int Bread::getUnits()
{
	return Units;
}

bool Bread::operator==(Bread& Compare)
{
	if (getName() == Compare.getName() && getPrice() == Compare.getPrice() && getType() == Compare.getType())
	{
		return true;
	}
	return false;
}

ostream& operator<<(ostream &out, Bread& Print)
{
	out << Print.getName() << " " << Print.getPrice() << " " << Print.getUnits() << " " << Print.sale() << " " << Print.gift() << ", Price: " << Print.price() << " Rs." << endl;
	return out;
}
